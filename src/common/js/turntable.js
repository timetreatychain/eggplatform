...
var createSvgEle = function (name) { return document.createElementNS('http://www.w3.org/2000/svg', name); };
...
var getPointsDistance = function (o, t) { return Math.sqrt(Math.pow(t.x - o.x, 2) + Math.pow(t.y - o.y, 2)); };
  ...
  this.values = this.values.map(function (info, i) {
    ...
    if (info.img) {
      ...
      var img = setAttrs(createSvgEle('image'), {
        ...
        transform: ("translate(" + (-(info.img.width / 2)) + ", " + (-(info.img.height / 2)) + ")")
      });
      g.appendChild(img);
    }
