    ...
    <div class="content">
      ...
      <div class="otcCenter otcCenter_setMes" v-if="view==3">
        ...
        <div class="phone">
          ...
          <input v-model="phonenum" class="phonenum" type="text" placeholder="输入手机号码">
...
export default {
  data() {
    return {
      ...
      datainfo: "",
      view: 1,
      otcusername: sessionStorage.otcAcount,
      zfburl: "",
      wxurl: "",
      upOTC: "",
      bidtprice: 0,
  ...
  watch: {
    view (val) {
      ...
      this.checkOtcBind();
      this.getupOTC();
      // this.otcusername = sessionStorage.otcAcount
      }
    }
  },
  methods: {
    ...
    //       vm.wxurl = data.data.userwximage,
    //       // type: Number($('input[name="gender"]:checked').val()) + 1,
    //        $("#checkbox")[0].checked= data.data.openstate == 1 ? true : false,
    //       vm.phonenum = data.data.phonenum
    ...
    sub() {
      let vm = this;
      $.ajax({
        url: contextPath1 + "/api/banding/regOTC",
        type: "post",
        async: true,
        dataType: "json",
        data: {
